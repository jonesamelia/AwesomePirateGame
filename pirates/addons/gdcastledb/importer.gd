...
func _generate_row_class( name:String, columns:Array ) -> String:
	...
	for column in columns:
		...
		match int( type[0] ):
			...
			CDBType.CDB_LIST:
				...
				init_code.append( "self.%s = _%s" % [column_name, column_name] )
				ref_code.append( "for t in self.%s: t._delay_set_reference( _import_src )" % [column_name] )
			CDBType.CDB_COLOR:
			...
			CDBType.CDB_REFERENCE:
				...
				# 参照解決コード
				ref_code.append( "self.%s = _import_src.table_%s.get_value(self._%s_ref_id)" % [column_name, sheet_inst_name, column_name] )
			_:
				# 未対応
				pass
	...
	code += "\t\tfunc _delay_set_reference( _import_src ):\n"
	if 0 < ref_code.size( ):
		for t in ref_code:
			code += "\t\t\t%s\n" % t
	else:
...
func _import( source_file:String, save_path:String, options:Dictionary, platform_variants:Array, gen_files:Array ) -> Error:
	...
	code += "\n"
	code += "# this file is auto generated by GDCastledb4 @arlez80\n"
